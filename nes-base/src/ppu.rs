...
pub struct PPUBusAdapter(pub Box<dyn PPU>);

impl Reader for PPUBusAdapter {
    fn read(&self, addr: u16) -> u8 {
        match (addr - 0x2000) % 8 {
            2 => self.0.read_reg_status(),
            4 => self.0.read_reg_oam_data(),
            7 => self.0.read_reg_data(),
...
impl Writer for PPUBusAdapter {
    fn write(&mut self, addr: u16, data: u8) {
        match (addr - 0x2000) % 8 {
            0 => self.0.write_reg_control(data),
            1 => self.0.write_reg_mask(data),
            3 => self.0.write_reg_oam_addr(data),
            4 => self.0.write_reg_oam_data(data),
            5 => self.0.write_reg_scroll(data),
            6 => self.0.write_reg_address(data),
            7 => self.0.write_reg_data(data),
